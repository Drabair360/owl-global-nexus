 ...
 * numéro romain elzévirien, titre en petites capitales, échelle symbolique,
 * et le folio-chouette en sceau de cartouche.
 */
const Cartouche = ({
  x,
  y,
  ...
  <g transform={`translate(${x} ${y})`} aria-hidden="true">
    ...
    <line x1="0" y1={h * 0.52} x2={w} y2={h * 0.52} stroke={ENCRE} strokeWidth={FIN} opacity="0.6" />
    <line x1={w - 62} y1="0" x2={w - 62} y2={h} stroke={ENCRE} strokeWidth={FIN} opacity="0.6" />

    <text className="gravure-lettrage" x="12" y={h * 0.34} fontSize="14">
      PLANCHE {numeral}
    </text>
    <text className="gravure-lettrage" x="12" y={h * 0.82} fontSize="13" fill={ENCRE} opacity="0.85">
      {title}
    </text>
    {echelle && (
      <text className="gravure-lettrage" x={w - 70} y={h * 0.34} fontSize="12" textAnchor="end">
        {echelle}
      </text>
    )}
